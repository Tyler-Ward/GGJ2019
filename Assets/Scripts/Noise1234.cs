...
//Modifcations made from basic C functions to work with Unity C# by GavstarB

using System.Collections;
using System.Collections.Generic;
using UnityEngine;

public static class Noise1234
{
    //---------------------------------------------------------------------
    // Static data
    ...
    static float grad1(int hash, float x)
    {
        ...
        if ((h & 8 ) >= 1) grad = -grad;         // and a random sign for the gradient
    ...
    static float grad4(int hash, float x, float y, float z, float t)
    {
        ...
        float v = h < 16 ? y : z;
        float w = h < 8 ? z : t;
        return (((h & 1) >= 1) ? -u : u) + (((h & 2) >= 1) ? -v : v) + (((h & 4) >= 1) ? -w : w);
    }
